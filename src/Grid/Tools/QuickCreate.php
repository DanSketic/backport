...
use DanSketic\Backport\Form\Field\MultipleSelect;
use DanSketic\Backport\Form\Field\Select;
use DanSketic\Backport\Form\Field\Text;
...
class QuickCreate implements Renderable
{
    ...
    protected function resolveView($class)
    {
        ...
        return "backport::grid.quick-create.{$name}";
    }
    ...
    $('.quick-create .create-form').submit(function (e) {
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (data.status == true) {
                    $.backport.toastr.success(data.message, '', {positionClass:"toast-top-center"});
                    $.backport.reload();
                    return;
                }
                
                if (typeof data.validation !== 'undefined') {
                    $.backport.toastr.warning(data.message, '', {positionClass:"toast-top-center"})
                }
            },
            error:function(XMLHttpRequest, textStatus){
                if (typeof XMLHttpRequest.responseJSON === 'object') {
                    $.backport.toastr.error(XMLHttpRequest.responseJSON.message, '', {positionClass:"toast-top-center", timeOut: 10000});
                }
            }
        });
...
SCRIPT;
    ...
    public function render($columnCount = 0)
    {
        ...
        return view('backport::grid.quick-create.form', $vars)->render();
    }
}
