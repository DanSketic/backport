...
use DanSketic\Backport\Grid\Selectable;
...
class BelongsTo extends AbstractDisplayer
{
    ...
    protected function getLoadUrl($selectable, $multiple = 0)
    {
        ...
        return route('backport.handle-selectable', compact('selectable', 'args'));
    }
    ...
    public function display($selectable = null, $column = '')
    {
        if (!class_exists($selectable) || !is_subclass_of($selectable, Selectable::class)) {
            throw new \InvalidArgumentException(
                "[Class [{$selectable}] must be a sub class of DanSketic\Backport\Grid\Selectable"
            );
        }

        return Backport::component('backport::grid.inline-edit.belongsto', [
