...
abstract class Extension
{
    ...
    public static function boot()
    {
        ...
        Backport::extend($extension->name, get_called_class());
    ...
    /**
     * Get config set in config/backport.php.
     *
     * @param string $key
     * @param null   $default
     *
    ...
    {
        $name = array_search(get_called_class(), Backport::$extensions);
    ...
    /**
     * Import menu item and permission to backport.
     */
    ...
    protected function getMenuValidationRules()
    {
        return [
            'title'    => 'required',
            'path'     => ['required', Rule::unique(Config::get('backport.database.menu_table'), 'uri')],
            'icon'     => 'required',
            'children' => 'nullable|array',
        ];
    }
    ...
    protected function getPermissionValidationRules()
    {
        return [
            'name'     => 'required',
            'slug'     => ['required', Rule::unique(Config::get('backport.database.permissions_table'), 'slug')],
            'path'     => 'required|array',
            'path.*'   => 'string',
    ...
    /**
     * Create a item in backport left side menu.
     *
     * @param string $title
     * @param string $uri
     * @param string $icon
