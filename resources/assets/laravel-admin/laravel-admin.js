...
(function ($) {
    ...
    $.admin = LA;
    $.backport.swal = swal;
    $.backport.toastr = toastr;
    $.backport.grid = new Grid();

    $.backport.reload = function () {
        $.pjax.reload('#pjax-container');
        $.backport.grid = new Grid();
    };

    $.backport.redirect = function (url) {
        $.pjax({container:'#pjax-container', url: url });
        $.backport.grid = new Grid();
    };

    $.backport.getToken = function () {
        return $('meta[name="csrf-token"]').attr('content');
    };

    $.backport.loadedScripts = [];

    $.backport.loadScripts = function(arr) {
        var _arr = $.map(arr, function(src) {

            if ($.inArray(src, $.backport.loadedScripts)) {
                return;
            }

            $.backport.loadedScripts.push(src);
